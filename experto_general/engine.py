...
# Obtener la siguiente pregunta basada en el índice actual del usuario
def get_next_question(knowledge_base, current_index):
    if current_index < len(knowledge_base["entries"]):
        return knowledge_base["entries"][current_index]["props"][0]
    return None

# Obtener el diagnóstico en base a los síntomas positivos del usuario
def get_diagnosis(knowledge_base, positive_symptoms):
    for entry in knowledge_base["entries"]:
        if all(symptom in positive_symptoms for symptom in entry["props"]):
            return entry["name"]
    return "Sin coincidencias en la base de conocimiento."
